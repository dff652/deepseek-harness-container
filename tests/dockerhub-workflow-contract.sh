...
grep -Fq 'tests/rc2-regression.sh' "$workflow" || fail 'rc.2 package regression missing from publication build'
grep -Fq 'tests/runtime-cve-reachability.sh' "$workflow" || fail 'runtime CVE evidence contract missing from publication preflight'
grep -Fq 'scripts/audit-loaded-runtime-cve.sh "$LOCAL_TAG"' "$workflow" || fail 'live built-image runtime CVE audit missing from publication gate'
grep -Fq 'runtime-cve-reachability.json gate' "$workflow" || fail 'live runtime CVE audit is not fail-closed before publication'
