...
readonly IMAGE_REF=${1:?usage: arm64-runtime.sh IMAGE_REF}
readonly CONTAINER_NAME="dsh-arm64-smoke-${$}-${RANDOM}"
readonly LABEL='io.deepseek-harness-container.test=arm64-runtime'
...
printf 'PASS: ARM64 rc.2 distroless DSH native modules, writable request-image state, no shell/package manager, version, loopback Web and non-root boundary\n'
